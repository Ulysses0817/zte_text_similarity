...
#   ps = json.load(f)
# output_path = ps["save_path"]
# print("output_path:", output_path)
# output_path = "s3://bucket-dyxubbh6/053e8653ed0010dc1f87c00e6b9c4cf6/5c52f2ffc5ee433b810695042fa7eb36/Debug/algo-aaa/output/"
# print("output_path:", output_path)
...
def tokenize_doc(_corpus, tokenizer, idx, i):
  all_documents = []
  for doc in _corpus:
    idx += 1
    if idx%1000 == 0:
      print(f"进度：{idx}-{idx/830606}")
    doc = doc.split(" 4 ")
    if len(doc) > 0:
  ...
  file_name = f"all{i}.json"
  print("save corpus to json file:", file_name)
  with open(file_name, "w") as f:
    json.dump(all_documents, f)
  print(os.listdir("./"))
  #os.remove(file_name)
  del all_documents, _corpus
  gc.collect()
...
def create_training_instances(input_files, tokenizer, max_seq_length,
  ...
  corpus=pd.read_table(r'./data/corpus.txt',sep='\t',header=None)
  print(corpus.shape)
  corpus = corpus[0].values
  total_length = len(corpus)
  sstep = total_length//3
  idx = 0
  for i in range(1):
    _left = i*sstep
    _right = _left + sstep
    print(f"{_left}-{_right}")
    tokenize_doc(corpus[_left:_right], tokenizer, idx, i) # 保存文件至"all0.json"
  del corpus
  gc.collect()
  ...
  for i in range(1):
    file_name = f"all{i}.json"
    print(file_name)
    with open(file_name, "r") as f:   # 读取文件"all0.json"
  ...
  for _ in range(dupe_factor):
    for document_index in range(len(all_documents)):
      if document_index%10000 == 0:
        print(f"{_}--进度：{document_index}-{document_index/830606}")
      instances.extend(
...
def main(_):
  ...
  print(FLAGS.dupe_factor, FLAGS.random_seed)
